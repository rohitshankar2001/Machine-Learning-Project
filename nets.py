...
class ResNet50(nn.Module):
    def __init__(self, image_size):
        super().__init__()
        self.name = "resNet"
        resNet50 = torch.hub.load('pytorch/vision:v0.10.0', 'resnet50', pretrained=False).cuda()
        self.resNet50NoFC = torch.nn.Sequential(*list(resNet50.children())[:-1]).cuda()
        # resNet50Summary = (summary(self.resNet50NoFC, (3, 224, 224)))

        x = torch.randn(2, 3, image_size, image_size).cuda()
        self.final_shape = self.resNet50NoFC(x).size()
        # print(self.final_shape)

        self.out_channels = self.final_shape[-3]
        self.resNet50NoFC = nn.DataParallel(self.resNet50NoFC, device_ids=[0,1,2]).cuda()
        # self.resNet50NoFC = self.resNet50NoFC.cuda(2)    

    def out_units(self, image_size):
        return self.final_shape[-3] * self.final_shape[-2] * self.final_shape[-1] 
    
    def out_size(self, image_size):
        return self.final_shape[-2] 

    def out_channels(self, image_size):
        return self.final_shape[-2] 

    def forward(self, x):
        x = self.resNet50NoFC(x)
        return x
...
class DeconvLayers(nn.Module):
    ...
    def __init__(self, image_channels=3, final_channels=16, depth=5, reducing_layers=None, batch_norm=True, nl="relu",
        ...
        if self.depth>0:
            ...
            # print(2**min(self.rl, self.depth-1))
